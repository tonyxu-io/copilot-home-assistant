# Archived source: `xitter`

Official x-cli usage is one execution path within X/Twitter workflows.

Original skill path: `/home/tonyxu/.hermes/skills/social-media/xitter`
...
## Tony-specific policy

Tony does **not** want Twitter/X API used for X automation anymore. For Tony's X actions, prefer the logged-in browser session via `bb-browser`/Chrome CDP. Do not call `x-cli` for posting or routine X actions unless Tony explicitly overrides this preference.
...
### Posts About the User's GitHub / Open-Source Work

When drafting or posting about Tony's work, PRs, or LinkedIn/profile positioning, do not write generic self-promotion from memory. Ground the copy in concrete contribution evidence first:

```bash
gh search prs --author tonyxu-io --limit 50 --json repository,title,url,state,createdAt,closedAt,number,body
gh pr view <number> --repo <owner/repo> --json number,title,state,url,body,additions,deletions,changedFiles,commits
```

Then extract specific claims: repo, PR number, status, what changed, why it mattered, and measurable scope such as files/additions/tests when useful. For posts about `gbrain`, `Hermes Agent`, Copilot, embeddings, or agent tooling, prefer this evidence-backed framing over vague phrases like "I use X for Y". If the user asks for LinkedIn/profile copy, draft it from these real PRs/contributions and do not update LinkedIn without explicit confirmation.
