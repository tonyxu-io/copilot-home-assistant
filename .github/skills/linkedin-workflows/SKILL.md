...
- **Browser posting:** use logged-in Chrome/CDP where available; verify the composer and final post state. See for the exact AX/CDP fallback that works when LinkedIn's SDUI composer ignores normal `bb-browser` clicks/fills.
...
- **Content backlog:** when Tony asks for a LinkedIn backlog, maintain `/home/tonyxu/brain/sources/LinkedIn/post-backlog.md`; for the gbrain import/verification workflow and the deeper agent theses.
